...
app.get('/api/certificate/generate', authenticateToken, async (req, res) => {
  try {
    ...
    let summaryUrl = `${EARNINGS_SERVICE_URL}/api/earnings/summary?`;
    if (start_date) summaryUrl += `start_date=${start_date}&`;
    if (end_date) summaryUrl += `end_date=${end_date}`;
    ...
    <body>
      <div class="container">
        <div class="header">
          ...
          <p>Period: ${start_date || 'Inception'} to ${end_date || 'Present'}</p>
        </div>
        ...
        <table class="earnings-table">
          ...
          <tbody>
            <tr>
              ...
              <td class="amount">${summary.total_gross.toLocaleString('en-US', {minimumFractionDigits: 2})}</td>
            </tr>
            <tr>
              <td>Platform Deductions (Commissions)</td>
              <td class="amount">-${summary.total_deductions.toLocaleString('en-US', {minimumFractionDigits: 2})}</td>
            </tr>
            <tr class="total-row">
              <td>Net Received Income</td>
              <td class="amount">${summary.total_net.toLocaleString('en-US', {minimumFractionDigits: 2})}</td>
            </tr>
          </tbody>
        </table>
        ...
        <div class="info-box" style="margin-bottom: 20px;">
          ...
          <p>Total Shifts Logged: ${summary.shift_count}</p>
          <p>Total Hours Worked: ${summary.total_hours}</p>
          <p>Average Hourly Rate: PKR ${summary.avg_hourly_rate.toLocaleString('en-US', {minimumFractionDigits: 2})}</p>
        </div>

        <div class="footer">
          <p>This document is generated by FairGig, a platform for gig worker income verification and rights advocacy.</p>
          <p>The numbers reflected above are based on data provided by the worker and ${summary.verified_count} shifts have been manually verified by FairGig labor advocates.</p>
