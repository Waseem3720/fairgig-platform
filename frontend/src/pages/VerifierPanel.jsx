...
const VerifierPanel = () => {
  ...
  const stats = {
    ...
    withEvidence: pending.filter(p => p.screenshot_url && p.screenshot_url !== "" && p.screenshot_url !== "null").length,
    noScreenshot: pending.filter(p => !p.screenshot_url || p.screenshot_url === "" || p.screenshot_url === "null").length,
  };

  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg-secondary)', padding: '0' }}>
      {/* Role-specific CSS vars aligned with index.css */}
      <style>{`
        :root {
          --vf-bg: var(--bg-secondary);
          --vf-surface: #ffffff;
          --vf-border: var(--border-light);
          --vf-accent: var(--accent-primary);
          --vf-accent-glow: rgba(16, 185, 129, 0.1);
          --vf-text: var(--text-primary);
          --vf-muted: var(--text-muted);
        }
        ...
        .vf-title { font-size: 2.2rem; font-weight: 800; color: var(--vf-text); margin-bottom: 4px; letter-spacing: -1px; }
        .vf-subtitle { color: var(--vf-muted); font-size: 0.95rem; font-weight: 500; }
        .vf-stats-row {
          display: flex;
          gap: 20px;
          padding: 24px 60px;
        ...
        .vf-stat-chip .vf-stat-number { font-size: 1.5rem; font-weight: 800; color: var(--vf-text); }
        .vf-stat-chip .vf-stat-label { font-size: 0.75rem; color: var(--vf-muted); font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; }
        .vf-body { padding: 40px 60px; }
        .vf-section-title {
          font-size: 0.85rem;
          font-weight: 800;
          color: var(--vf-muted);
        ...
        .vf-shift-id { font-size: 0.8rem; color: var(--vf-muted); font-family: monospace; font-weight: 700; }
        .vf-info-grid {
          display: grid;
        ...
        .vf-info-item label { display: block; font-size: 0.65rem; color: var(--vf-muted); text-transform: uppercase; letter-spacing: 1px; margin-bottom: 5px; font-weight: 800; }
        .vf-info-item span { font-size: 1rem; color: var(--vf-text); font-weight: 600; }
        .vf-evidence-box {
        ...
        .vf-btn-approve:hover:not(:disabled) { background: rgba(16,185,129,0.15); border-color: #10b981; }
        .vf-btn-reject {
          background: rgba(239,68,68,0.08);
          color: #dc2626;
          border-color: rgba(239,68,68,0.1);
        }
        .vf-btn-reject:hover:not(:disabled) { background: rgba(239,68,68,0.15); border-color: #ef4444; }
        .vf-btn-flag {
          background: rgba(245,158,11,0.08);
          color: #d97706;
          border-color: rgba(245,158,11,0.1);
        }
        .vf-btn-flag:hover:not(:disabled) { background: rgba(245,158,11,0.15); border-color: #f59e0b; }
        .vf-empty {
          text-align: center;
          padding: 80px 20px;
          color: #475569;
        ...
        <div className="vf-page">
          ...
          <div className="vf-stats-row">
            ...
            <div className="vf-stat-chip">
              ...
              <div>
                <div className="vf-stat-number">{stats.withEvidence}</div>
            ...
            <div className="vf-stat-chip">
              ...
              <div>
                <div className="vf-stat-number">{stats.noScreenshot}</div>
                  ...
                  {pending.map((shift, idx) => (
                    ...
                    >
                      <div className="vf-card-header">
                        <span className={`badge ${shift.screenshot_url ? 'badge-pending' : 'badge-danger'}`}>
                          {shift.screenshot_url ? 'PENDING REVIEW' : 'MISSING EVIDENCE'}
                        </span>
                      ...
                      {shift.screenshot_url ? (
                        <div className="vf-evidence-box" style={{ borderColor: 'var(--vf-accent)' }}>
                          <Eye size={15} color="#6366f1" />
                          <button
                            className="vf-evidence-link"
                            onClick={() => {
                              const url = shift.screenshot_url.startsWith('http') 
                                ? shift.screenshot_url 
                                : `http://localhost:8002${shift.screenshot_url}`;
                              setPreviewUrl(url);
                            }}
                          >
                        ...
                        <div className="vf-evidence-box" style={{ borderStyle: 'solid', borderColor: 'rgba(239,68,68,0.3)', background: 'rgba(239,68,68,0.05)' }}>
                          <AlertTriangle size={15} color="#ef4444" />
                          <span style={{ fontSize: '0.85rem', color: '#ef4444', fontWeight: 500 }}>No screenshot submitted</span>
                        </div>
                      )}
